...
![the leaked admin session cookie](04-admin-cookie.png)
...
![authenticated admin dashboard](06-admin-dashboard.png)
...
![whitespace filter bypass with ${IFS}](08-ifs-bypass.png)
![time-based injection confirmed](09-time-based.png)
...
![building the base64 reverse shell payload](10-b64-payload.png)

Caught a shell as the `app` service account:

![reverse shell as app](11-app-shell.png)
...
![the application JAR](12-jar.png)
![downloading the JAR to Kali](13-download-jar.png)

`application.properties` inside the JAR held a **PostgreSQL** connection
string:

![postgres credentials in application.properties](14-postgres-creds.png)
...
![psql connected](15-psql.png)
![users table](16-users-table.png)

The `users` table held a bcrypt hash for the `admin` account:

![admin bcrypt hash](17-admin-bcrypt.png)
...
![SSH as josh](19-ssh-josh.png)
![user flag](20-user-flag.png)
...
![abusing ssh ProxyCommand](22-proxycommand.png)
![root shell](23-root-shell.png)

Read the final flag from `/root/root.txt`:

![root flag](24-root-flag.png)
...
**See also:** [Linux privilege escalation methodology](https://github.com/debasjan/security-portfolio/blob/main/methodology/linux-privesc.md)
