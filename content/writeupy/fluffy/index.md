...
![responder capturing p.agila's NetNTLMv2 hash](07-responder-ntlm-capture.png)
...
![BloodHound collection run](03-bloodhound.png)

The graph showed `p.agila` belongs to a group with **`GenericAll`** over
a `service accounts` group:

![GenericAll from service account managers onto service accounts](08-genericall-service-accounts.png)
...
![shadow-credential attack recovering winrm_svc's NT hash](10-shadow-creds-winrm-hash.png)
...
![updating ca_svc's UPN to administrator](11-upn-updated-to-administrator.png)

```bash
certipy-ad req -u ca_svc -hashes <RC4_HASH> -dc-ip 10.129.232.88 -target dc01.fluffy.htb -ca fluffy-DC01-CA -template User
```

![requesting the certificate as ca_svc](12-certipy-req-administrator-pfx.png)
...
![certipy auth recovering the Administrator NT hash](13-certipy-auth-admin-hash.png)
